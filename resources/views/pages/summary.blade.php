                ...
                <div class="col-xl-4 col-sm-6 mb-3">
                    <div class="card text-white {{($result['type'] == 'income')? 'bg-info':'bg-danger'}} o-hidden h-100">
                        <div class="card-header">
                            <span class="float-left text-dark">{{($result['type'] == 'income')? $result['income_date']:$result['expense_date']}}</span>
                        </div>
                        <div class="card-body">
                            ...
                            <div>{{($result['type'] == 'income')? '€ '.$result['income_date']: ' '.$result['expense_date']}}</div>
                        </div>
                    </div>
                </div>
            @endforeach
